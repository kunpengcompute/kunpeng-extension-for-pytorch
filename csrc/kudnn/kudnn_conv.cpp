    ...
    if (py::isinstance<py::int_>(param)) {
        ...
        if (temp_vec.size() != static_cast<size_t>(target_dim)) {
            throw std::invalid_argument(
                "Collection length does not match target dimension: expected " +
        ...
        for (int i = 0; i < target_dim; ++i) {
            if (temp_vec[i] < 0) {
                throw std::invalid_argument(err_non_neg + ", got " + std::to_string(temp_vec[i]) + " at index " + std::to_string(i));
            }
        }
        ...
        throw std::invalid_argument(
            "Unsupported param type: only support int, tuple, list"
        );
    }
                                                   ...
                                                   const std::vector<int64_t>& kernel_size = {})
{
    ...
    if (py::isinstance<py::str>(param)) {
        ...
        if (padding_str == "valid") {
            ...
            throw std::invalid_argument(
                "Unsupported padding str: only 'valid' or 'same' are allowed"
            );
        }
    } else {
...
std::vector<int64_t> python_padding_to_int64vector_2d(py::object param, 
                                                      const std::vector<int64_t>& stride_vec, 
...
std::vector<int64_t> python_padding_to_int64vector_3d(py::object param, 
                                                      const std::vector<int64_t>& stride_vec,
                                                      const at::Tensor& weight, 
                                                      const std::vector<int64_t>& dilation_vec)
{
...
static inline std::vector<int64_t> getOutSize(const at::Tensor &input, const at::Tensor &weight, const std::vector<int64_t>& strideVec,
                                              const Shape& paddingL, const Shape& paddingR, const std::vector<int64_t>& dilationVec,
                                              int64_t dim)
{
    ...
    if (dim == 2) {
        ...
        const auto OH = getOutShapeByPadding(IH, KH, strideVec[0], dilationVec[0], 
                                             static_cast<int64_t>(paddingL[0]), static_cast<int64_t>(paddingR[0]));
        const auto OW = getOutShapeByPadding(IW, KW, strideVec[1], dilationVec[1], 
                                             static_cast<int64_t>(paddingL[1]), static_cast<int64_t>(paddingR[1]));
        ...
        const auto OD = getOutShapeByPadding(ID, KD, strideVec[0], dilationVec[0], 
                                             static_cast<int64_t>(paddingL[0]), static_cast<int64_t>(paddingR[0]));
        const auto OH = getOutShapeByPadding(IH, KH, strideVec[1], dilationVec[1], 
                                             static_cast<int64_t>(paddingL[1]), static_cast<int64_t>(paddingR[1]));
        const auto OW = getOutShapeByPadding(IW, KW, strideVec[2], dilationVec[2], 
                                             static_cast<int64_t>(paddingL[2]), static_cast<int64_t>(paddingR[2]));
                       ...
                       const std::vector<int64_t>& dilation_vec, int64_t groups)
{
    ...
    if (padding_vec.size() != static_cast<size_t>(dim)) {
        throw std::invalid_argument("Invalid padding size. Expected " + std::to_string(dim) + 
                                    " elements, got " + std::to_string(padding_vec.size()));
    }
...
static std::tuple<at::Tensor, bool> batchify(const at::Tensor& input, const int64_t num_spatial_dims, const std::string& func_name) 
{
...
at::Tensor kudnn_conv2d(
    ...
    if (bias.defined() && bias.dtype() != input.dtype()) {
        throw std::invalid_argument("Input type (" + std::string(input.dtype().name()) + 
                                    ") and bias type (" + std::string(bias.dtype().name()) + ") should be the same");
    }
